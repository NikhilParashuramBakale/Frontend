...
const BatDetailsPage: React.FC = () => {
  ...
  console.log('BatDetailsPage - batId:', batId);
  console.log('BatDetailsPage - loading:', loading);
  console.log('BatDetailsPage - navigationState:', navigationState);
  console.log('BatDetailsPage - isExpanded:', isExpanded);
      ...
      <div className={`max-w-7xl mx-auto px-4 py-8 transition-all duration-300 ${isExpanded ? 'ml-64' : 'ml-16'}`}>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="space-y-2 text-sm">
            <p><strong>BAT ID:</strong> {batId}</p>
            <p><strong>Loading:</strong> {loading.toString()}</p>
            <p><strong>Sensor Data:</strong> {sensorData ? 'Available' : 'None'}</p>
            <p><strong>Spectrogram URL:</strong> {spectrogramUrl ? 'Available' : 'None'}</p>
            <p><strong>Camera URL:</strong> {cameraUrl ? 'Available' : 'None'}</p>
            <p><strong>Navigation State:</strong> {navigationState ? JSON.stringify(navigationState) : 'None'}</p>
          </div>
        </div>
